/* ── Theme ── */
        ...
        function initCollapsibles() {
            document.querySelectorAll('.section-title').forEach(el => {
                ...
                el.addEventListener('click', () => {
                    ...
                    while (next && !next.classList.contains('section-title') && next.tagName !== 'SCRIPT' && next.tagName !== 'STYLE') {
        ...
        function handleFile(file) {
            document.getElementById('upload-title').textContent = 'Loading CSV into memory...';
            document.getElementById('upload-desc').textContent = 'This might take a few seconds with large files.';

            Papa.parse(file, {
                header: true,
                skipEmptyLines: true,
                worker: true, // Parse in background thread so UI doesn't freeze
                complete: results => {
                    rawParsedData = results.data;
                    document.getElementById('upload-title').textContent = 'Calculating Stats...';
                    setTimeout(() => processData(rawParsedData), 50); // Small breath for UI to repaint
                },
                error: () => {
                    alert('Could not parse that file. Make sure it\'s a valid DiscordChatExporter CSV.');
                    location.reload();
                }
            });
        }
        ...
        document.querySelectorAll('.user-table th').forEach(th => {
            th.addEventListener('click', () => {
                ...
                else { sortCol = col; sortAsc = (col === 'name' || col === 'resp'); }
                
                document.querySelectorAll('.user-table th').forEach(h => h.className = '');
        ...
        function processData(data) {
            ...
            const filterAfter = filterAfterVal ? new Date(filterAfterVal + 'T00:00:00') : null;
            const filterBefore = filterBeforeVal ? new Date(filterBeforeVal + 'T23:59:59') : null;
            ...
            if (filterAfter || filterBefore) {
                filterCountEl.textContent = fmtNum(validData.length) + ' of ' + fmtNum(totalRaw) + ' messages';
            } else {
            ...
            const authorStats = {};
            allAuthors.forEach(a => authorStats[a] = {
                msgs: 0, wordCounts: [], responseTimes: [], starters: 0,
                media: 0, consecutiveStreaks: [], emojis: 0, stickers: 0,
                voiceMessages: 0, calls: 0, callMinutes: 0, callDurations: [],
                reactions: 0, totalWords: 0
            });
            ...
            const stopWords = new Set([
                "the","and","to","i","a","of","it","in","you","is","for","that","my","on","me","we","this","with",
                "so","be","but","was","have","not","your","like","just","are","do","im","can","what","if","all",
                "get","out","about","when","know","up","how","they","as","at","it's","i'm","don't","dont","or",
                "from","no","he","she","would","got","too","ur","u","its","that's","then","there","was","were",
                "been","will","going","really","some","because","cuz","cause","https","http","com","www","net","org",
                "png","jpg","jpeg","gif","mp4","mov","webp","discordapp","cdn"
            ]);

            const customEmojiRegex = /<:[^:]+:\d+>/g;
            const stickerRegex = /\[([^\]]+)\]\(https:\/\/media\.discordapp\.net\/stickers\//;
            ...
            validData.forEach(row => {
                ...
                // ── Media & Links ──
                if ((attachments.trim() !== "" && !attachments.includes('voice-message.ogg')) || content.includes('http://') || content.includes('https://')) {
                ...
                if (reactionsStr.trim()) {
                    ...
                    if (msgReactionCount > 0) {
                        ...
                        userReactionsReceived[author] = (userReactionsReceived[author] || 0) + msgReactionCount;
                ...
                // ── Words ──
                const rawWords = content.trim().split(/\s+/).filter(w => w.length > 0);
                const wc = rawWords.length;
                authorStats[author].wordCounts.push(wc);
                authorStats[author].totalWords += wc;
                totalWords += wc;

                let cleanContent = content.replace(/https?:\/\/[^\s]+/g, '').replace(/<:[^:]+:\d+>/g, '');
            ...
            for (let i = 0; i < sortedDates.length; i++) {
                ...
                if (i === 0) { currentStreak = 1; }
                    ...
                    else { if (currentStreak > maxStreak) maxStreak = currentStreak; currentStreak = 1; }
                }
            }
            ...
            Object.keys(authorStats).forEach(author => {
                ...
                if (authorStats[author].msgs > topSender.count) topSender = { name: author, count: authorStats[author].msgs };
                if (authorStats[author].starters > topStarter.count) topStarter = { name: author, count: authorStats[author].starters };
            });
            ...
            const statsData = [
                ...
                { label: 'Top Sender', value: topSender.name, sub: fmtNum(topSender.count) + ' msgs' },
                { label: 'Starts Most Chats', value: topStarter.name, sub: fmtNum(topStarter.count) + ' times' },
                { label: 'Busiest Day', value: busiestDay.date, sub: fmtNum(busiestDay.count) + ' msgs' },
            ...
            if (totalReactions > 0) {
                ...
                const topReactor = Object.entries(userReactionsReceived).sort((a,b) => b[1] - a[1])[0];
                const reactionStats = [
                    { label: 'Total Reactions', value: fmtNum(totalReactions) },
                    { label: 'Reacted Messages', value: fmtNum(totalReactedMessages) },
                    { label: 'Unique Reactions', value: fmtNum(Object.keys(reactionCounts).length) },
                    { label: 'Most Reacted User', value: topReactor ? topReactor[0] : '—', sub: topReactor ? fmtNum(topReactor[1]) + ' reactions' : '' },
                ];

                document.getElementById('stats-reactions').innerHTML = reactionStats.map(s =>
                    `<div class="stat-card">
                        <div class="label">${s.label}</div>
                        <div class="value" title="${s.value}">${s.value}</div>
                        ${s.sub ? `<div class="sub">${s.sub}</div>` : ''}
            ...
            if (totalCalls > 0) {
                ...
                const callStats = [
                    ...
                    { label: 'Average Call', value: fmtDuration(Math.round(parseFloat(avgCallDur))) },
                    { label: 'Longest Call', value: fmtDuration(longestCall) },
                    { label: 'Shortest Call', value: fmtDuration(shortestCall) },
                    { label: 'Most Active Caller', value: topCaller.name, sub: fmtNum(topCaller.count) + ' calls' },
                ];

                document.getElementById('stats-calls').innerHTML = callStats.map(s =>
                    `<div class="stat-card">
                        <div class="label">${s.label}</div>
                        <div class="value" title="${s.value}">${s.value}</div>
                        ${s.sub ? `<div class="sub">${s.sub}</div>` : ''}
        ...
        function renderTable() {
            ...
            const sortedEntries = Object.entries(lastAuthorStats).sort((a, b) => {
                ...
                switch(sortCol) {
                    ...
                    case 'msgs': valA = sA.msgs; valB = sB.msgs; break;
                    case 'share': valA = sA.msgs; valB = sB.msgs; break; // share correlates exactly with msgs
                    case 'words': 
                        ...
                        break;
                    case 'resp':
                        valA = sA.responseTimes.length ? sA.responseTimes.reduce((x,y)=>x+y,0)/sA.responseTimes.length : 999999;
                        valB = sB.responseTimes.length ? sB.responseTimes.reduce((x,y)=>x+y,0)/sB.responseTimes.length : 999999;
                        break;
                    case 'starts': valA = sA.starters; valB = sB.starters; break;
                    case 'media': valA = sA.media; valB = sB.media; break;
                    case 'emojis': valA = sA.emojis; valB = sB.emojis; break;
                    case 'calls': valA = sA.calls; valB = sB.calls; break;
                    case 'calltime': valA = sA.callMinutes; valB = sB.callMinutes; break;
                }
            ...
            tbody.innerHTML = sortedEntries.map(([name, s]) => {
                const pct = lastTotalProcessedMsgs > 0 ? ((s.msgs / lastTotalProcessedMsgs) * 100).toFixed(1) : '0';
                const avgWords = s.wordCounts.length > 0 ? (s.totalWords / s.wordCounts.length).toFixed(1) : '0';
                const avgResp = s.responseTimes.length > 0
                    ? (s.responseTimes.reduce((a,b) => a+b, 0) / s.responseTimes.length).toFixed(1) + ' min'
                    : '—';
        ...
        function renderPieChart(authorStats, canvasId, dataKey) {
            ...
            const labels = Object.keys(authorStats).filter(a => authorStats[a][dataKey] > 0);
            const data = labels.map(a => authorStats[a][dataKey]);
            charts[canvasId] = new Chart(ctx, {
                type: 'doughnut',
                data: {
                    labels,
                    datasets: [{ data, backgroundColor: palette.slice(0, labels.length), borderColor: '#2b2d31', borderWidth: 3, hoverOffset: 6 }]
                },
                options: {
                    responsive: true, maintainAspectRatio: false,
        ...
        function renderDoubleTextChart(authorStats) {
            ...
            const labels = Object.keys(authorStats).filter(a => authorStats[a].consecutiveStreaks.length > 0);
            ...
            charts['doubleTextChart'] = new Chart(ctx, {
                type: 'bar',
                data: { labels, datasets: [{ label: 'Avg consecutive', data, backgroundColor: palette[0], borderRadius: 3 }] },
                options: barOpts()
            });
        }
        ...
        function renderTimePeriodChart(timePeriodCounts) {
            ...
            const labels = ['Night (12–6 AM)', 'Morning (6 AM–12 PM)', 'Afternoon (12–6 PM)', 'Evening (6 PM–12 AM)'];
            const data = [timePeriodCounts.night, timePeriodCounts.morning, timePeriodCounts.afternoon, timePeriodCounts.evening];
            charts['timePeriodChart'] = new Chart(ctx, {
                type: 'polarArea',
                data: {
                    labels,
                    datasets: [{ data, backgroundColor: palette.slice(0, 4).map(c => c + 'aa'), borderColor: '#2b2d31', borderWidth: 2 }]
                },
                options: {
                    responsive: true, maintainAspectRatio: false,
                    plugins: { legend: { position: 'bottom', labels: { color: tickColor, font: { size: 11 }, padding: 10 } } },
        ...
        function renderMsgLengthChart(authorStats) {
            ...
            const labels = Object.keys(authorStats).filter(a => authorStats[a].msgs > 0);
            ...
            charts['msgLengthChart'] = new Chart(ctx, {
                ...
                data: {
                    labels,
                    datasets: [
                        { label: 'Average', data: avgData, backgroundColor: palette[0], borderRadius: 3 },
                        { label: 'Median', data: medianData, backgroundColor: palette[4], borderRadius: 3 }
                    ]
                },
                options: {
                    responsive: true, maintainAspectRatio: false,
        ...
        function renderResponseTimeChart(authorStats) {
            ...
            const labels = Object.keys(authorStats).filter(a => authorStats[a].responseTimes.length > 0);
            const data = labels.map(a => {
            ...
            charts['responseTimeChart'] = new Chart(ctx, {
                type: 'bar',
                data: { labels, datasets: [{ label: 'Minutes', data, backgroundColor: palette[3], borderRadius: 3 }] },
                options: barOpts()
            });
        }
        ...
        function renderWordsChart(wordCounts) {
            ...
            charts['wordsChart'] = new Chart(ctx, {
                type: 'bar',
                data: { labels: sorted.map(i => i[0]), datasets: [{ data: sorted.map(i => i[1]), backgroundColor: palette[1], borderRadius: 3 }] },
                options: {
                    indexAxis: 'y', responsive: true, maintainAspectRatio: false,
                    plugins: { legend: { display: false } },
                    scales: {
                        x: { grid: { color: gridColor }, ticks: { color: tickColor } },
                        y: { grid: { display: false }, ticks: { color: '#f2f3f5', font: { weight: '600', size: 11 } } }
                    }
                }
            });
        }
        ...
        function renderEmojiChart(emojiCounts) {
            ...
            const sorted = Object.entries(emojiCounts).sort((a, b) => b[1] - a[1]).slice(0, 12);
            if (sorted.length === 0) {
            ...
            charts['emojiChart'] = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{ data: sorted.map(i => i[1]), backgroundColor: palette[4], borderRadius: 3 }]
                },
                options: {
                    indexAxis: 'y', responsive: true, maintainAspectRatio: false,
                    plugins: { legend: { display: false } },
        ...
        function renderTimelineChart(dateCounts) {
            ...
            charts['timelineChart'] = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Messages', data,
                        borderColor: palette[0], backgroundColor: 'rgba(88,101,242,0.15)',
                        borderWidth: 1.5, fill: true, tension: 0.3, pointRadius: data.length > 30 ? 0 : 2
                    }]
                },
                options: {
                    responsive: true, maintainAspectRatio: false,
        ...
        function renderDayOfWeekChart(dayOfWeekCounts) {
            ...
            charts['dayOfWeekChart'] = new Chart(ctx, {
                ...
                data: {
                    labels,
                    datasets: [{
                        label: 'Messages', data: dayOfWeekCounts,
                        backgroundColor: 'rgba(88,101,242,0.25)', borderColor: palette[0],
                        pointBackgroundColor: palette[4], borderWidth: 1.5
                    }]
                },
                options: {
                    responsive: true, maintainAspectRatio: false,
                    ...
                    scales: {
                        r: {
                            ...
                            pointLabels: { color: '#f2f3f5', font: { size: 12 } }, ticks: { display: false }
                        }
                    }
                }
            });
        ...
        function renderTimeOfDayChart(hourCounts) {
            ...
            charts['timeOfDayChart'] = new Chart(ctx, {
                type: 'bar',
                data: { labels, datasets: [{ label: 'Messages', data: hourCounts, backgroundColor: palette[5], borderRadius: 2 }] },
        ...
        function renderCallDurationDistChart(allMins) {
            ...
            const buckets = {
                '< 5m': 0, '5–15m': 0, '15–30m': 0, '30m–1h': 0,
                '1–2h': 0, '2–4h': 0, '4h+': 0
            };
            allMins.forEach(m => {
                if (m < 5) buckets['< 5m']++;
            ...
            charts['callDurationDistChart'] = new Chart(ctx, {
                ...
                data: {
                    labels,
                    datasets: [{ label: 'Calls', data, backgroundColor: palette[1], borderRadius: 3 }]
                },
                options: barOpts()
            });
        }
        ...
        function renderCallTimelineChart(allCallDurations) {
            ...
            const labels = allCallDurations.map(c => c.date.toISOString().split('T')[0]);
            ...
            charts['callTimelineChart'] = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        borderColor: palette[4], backgroundColor: 'rgba(235,69,158,0.15)',
                        borderWidth: 1.5, fill: true, tension: 0.2,
                        pointRadius: pointSizes, pointBackgroundColor: palette[4],
                        pointHoverRadius: 8
                    }]
                },
                options: {
                    ...
                    scales: {
                        ...
                        y: {
                            ...
                            title: { display: true, text: 'Duration', color: tickColor, font: { size: 11 } }
                        }
                    },
                    interaction: { mode: 'nearest', axis: 'x', intersect: false }
                }
        ...
        function renderCallDayOfWeekChart(callDayOfWeekCounts) {
            ...
            charts['callDayOfWeekChart'] = new Chart(ctx, {
                ...
                data: {
                    labels,
                    datasets: [{ label: 'Calls', data: callDayOfWeekCounts, backgroundColor: palette[0], borderRadius: 3 }]
                },
                options: barOpts()
            });
        }
        function renderHeatmap(heatmapData) {
            ...
            legend.innerHTML = 'Less ' + steps.map(s => {
                const bg = s === 0 ? 'var(--grid-empty)' : `rgba(88,101,242,${0.15 + s * 0.85})`;
        ...
        function renderTopReactionsChart(reactionCounts) {
            ...
            const sorted = Object.entries(reactionCounts).sort((a, b) => b[1] - a[1]).slice(0, 12);
            charts['topReactionsChart'] = new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: sorted.map(i => i[0]),
                    datasets: [{ data: sorted.map(i => i[1]), backgroundColor: palette[3], borderRadius: 3 }]
                },
                options: {
                    indexAxis: 'y', responsive: true, maintainAspectRatio: false,
                    plugins: { legend: { display: false } },
        ...
        function renderReactedUsersChart(userReactionsReceived) {
            ...
            const sorted = Object.entries(userReactionsReceived).sort((a, b) => b[1] - a[1]).slice(0, 6);
